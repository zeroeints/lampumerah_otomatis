"""
sumo_simulation_opt.py — Versi OPTIMASI dari sumo_simulation.py
=============================================================================
Perubahan dari versi asli:
  1. Logika Fuzzy diaktifkan penuh untuk simpang 2-fase (sebelumnya di-bypass)
  2. Green Extension diaktifkan untuk simpang 2-fase (sebelumnya diblokir)
  3. Green Cut diaktifkan untuk simpang 2-fase (sebelumnya diblokir)
  4. Hard cap durasi hijau (44s) dihapus, menggunakan max_green penuh
  5. Threshold extension/cut diturunkan agar lebih responsif
  6. Import fuzzy_controller_opt (2 input: kendaraan + antrean, 9 rules)
  7. Pembagi pressure diturunkan dari 3.8 ke 2.8 (lebih sensitif)
  8. Queue length dikirim ke fuzzy.infer() sebagai input kedua

Cara mengembalikan ke versi asli:
  Di app_yolo11_gui.py, ganti:
        ...
        return [
            {
                "name": "NS_MAIN",
                "dirs": ["Utara", "Selatan"],
                "green_state": "GGrrGGrr",
                "yellow_state": "yyrryyrr",
                "notes": "Arus vertikal sebagai satu fase penuh.",
            },
            {
                "name": "EW_MAIN",
                "dirs": ["Timur", "Barat"],
                "green_state": "rrGGrrGG",
                "yellow_state": "rryyrryy",
...
def _decide_green_duration(
    ...
    fuzzy_input = int(max(0, min(30, round(pressure / 2.8))))
    queue_input = int(max(0, min(50, round(phase_queue))))
    inferred, _detail = fuzzy.infer(fuzzy_input, queue_input)
...
def _run_single_mode(
    *,
    ...
    sumo_phase_states = {}
    try:
        logics = traci.trafficlight.getAllProgramLogics(resolved_tl_id)
        if logics and logics[0].phases:
            for idx, p in enumerate(logics[0].phases):
                sumo_phase_states[idx] = p.state
    except Exception:
        pass

    def get_current_state_str(p_dict: Dict, c_stage: str) -> str:
        if "green_state" in p_dict:
            if c_stage == "GREEN":
                return p_dict["green_state"]
            elif c_stage == "YELLOW":
                return p_dict["yellow_state"]
            else:
                return "r" * len(p_dict["green_state"])
        else:
            if c_stage == "GREEN":
                p_idx = p_dict["green_phase"]
            elif c_stage == "YELLOW":
                p_idx = p_dict["yellow_phase"]
            else:
                return "r" * controlled_links
            return sumo_phase_states.get(p_idx, "r" * controlled_links)
    ...
    starvation_limit = max(max_green + yellow + all_red + 10, 75)
    ...
    try:
        for step in range(duration_steps):
            # Override state at every step to enforce manual control
            c_state_str = get_current_state_str(phase, stage)
            traci.trafficlight.setRedYellowGreenState(resolved_tl_id, c_state_str)
            ...
            if timer > 0:
                if stage == "GREEN" and mode == "fuzzy":
                    ...
                    can_extend = (
                        ...
                        and (q_now >= 8.0 or sum(flow_dir.values()) >= 10.0)
                    )
                    ...
                    if forced_next_idx is not None and timer > 2:
                        ...
                        and (this_queue < 4.0 or this_dir_peak < 2.0)
                        and timer > 4
                    ):
                        timer = 1
                        telemetry.record_phase_event(
            ...
            if stage == "GREEN":
                ...
                if mode == "fuzzy":
                    ...
                    if len(phase_plan) == 2:
                        # OPT: Gunakan output Fuzzy langsung untuk simpang 2-fase
                        # agar durasi hijau responsif terhadap antrean real-time.
                        ...
                        # Bias tambahan berdasarkan selisih antrean antar fase
                        queue_bias = int(round(max(-4.0, min(6.0,
                            ((phase_queue - other_queue) * 0.35)
                            + ((_phase_direction_peak(phase, q_dir) - other_peak) * 0.25)
                        ))))
                        current_green = int(round(
                            (0.65 * fuzzy_green) + (0.35 * ratio_green)
                            + starvation_boost + queue_bias
                        ))
                    else:
                        current_green = int(round((0.7 * fuzzy_green) + (0.3 * ratio_green) + starvation_boost))
                    current_green = min(current_green, max_green)
